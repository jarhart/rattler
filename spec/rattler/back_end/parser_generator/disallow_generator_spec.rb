...
describe Rattler::BackEnd::ParserGenerator::DisallowGenerator do
  ...
  describe '#gen_dispatch_action' do
    ...
    context 'when nested' do
      it 'generates nested negative lookahead code with a dispatch action' do
        nested_code {|g| g.gen_dispatch_action disallow, 'Word', 'parsed' }.
          should == (<<-CODE).strip
begin
  @scanner.skip(/(?!\\d)/) &&
  Word.parsed([])
    ...
    context 'when top-level' do
      it 'generates top level negative lookahead code with a dispatch action' do
        top_level_code {|g| g.gen_dispatch_action disallow, 'Word', 'parsed' }.
          should == (<<-CODE).strip
@scanner.skip(/(?!\\d)/) &&
Word.parsed([])
          CODE
  ...
  describe '#gen_direct_action' do
    ...
    context 'when nested' do
      it 'generates nested negative lookahead code with a direct action' do
        nested_code {|g| g.gen_direct_action disallow, ActionCode.new(':t') }.
          should == (<<-CODE).strip
begin
  @scanner.skip(/(?!\\d)/) &&
  (:t)
    ...
    context 'when top-level' do
      it 'generates top level negative lookahead code with a direct action' do
        top_level_code {|g| g.gen_direct_action disallow, ActionCode.new(':t') }.
          should == (<<-CODE).strip
@scanner.skip(/(?!\\d)/) &&
(:t)
          CODE
