...
describe Rattler::BackEnd::ParserGenerator::DirectActionGenerator do
  ...
  describe '#gen_direct_action' do
    ...
    context 'when nested' do
      it 'generates nested matching code with nested direct actions' do
        nested_code {|g| g.gen_direct_action action, ActionCode.new('|_| _.size') }.
          should == (<<-CODE).strip
begin
  (r = begin
    (r = @scanner.scan(/\\w+/)) &&
  ...
  (r.size)
    ...
    context 'when top-level' do
      it 'generates top level matching code with nested direct actions' do
        top_level_code {|g| g.gen_direct_action action, ActionCode.new('|_| _.size') }.
          should == (<<-CODE).strip
(r = begin
  (r = @scanner.scan(/\\w+/)) &&
  (r.to_sym)
