...
describe Rattler::BackEnd::ParserGenerator::LabelGenerator do
  ...
  describe '#gen_dispatch_action' do
    ...
    context 'when nested' do
      it 'generates nested matching code with a dispatch action' do
        nested_code {|g| g.gen_dispatch_action label, 'Word', 'parsed' }.
          should == (<<-CODE).strip
begin
  (r = @scanner.scan(/\\w+/)) &&
  Word.parsed([r])
    ...
    context 'when top-level' do
      it 'generates top level matching code with a dispatch action' do
        top_level_code {|g| g.gen_dispatch_action label, 'Word', 'parsed' }.
          should == (<<-CODE).strip
(r = @scanner.scan(/\\w+/)) &&
Word.parsed([r])
          CODE
  ...
  describe '#gen_direct_action' do
    ...
    context 'when nested' do
      it 'generates nested matching code with a direct action' do
        nested_code {|g| g.gen_direct_action label, ActionCode.new('|_| _.to_sym') }.
          should == (<<-CODE).strip
begin
  (r = @scanner.scan(/\\w+/)) &&
  (r.to_sym)
    ...
    context 'when top-level' do
      it 'generates top level matching code with a direct action' do
        top_level_code {|g| g.gen_direct_action label, ActionCode.new('|_| _.to_sym') }.
          should == (<<-CODE).strip
(r = @scanner.scan(/\\w+/)) &&
(r.to_sym)
          CODE
