# Generated by jeweler
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Jeweler::Tasks in Rakefile, and run 'rake gemspec'
# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = "rattler"
  s.version = "0.6.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.authors = ["Jason Arhart"]
  s.date = "2012-06-28"
  s.description = "Simple language recognition tool for Ruby based on packrat parsing"
  s.email = "jarhart@gmail.com"
  s.executables = ["rtlr"]
  s.extra_rdoc_files = [
    "LICENSE.txt",
    "README.rdoc"
  ]
  s.files = [
    "bin/rtlr",
    "lib/rattler.rb",
    "lib/rattler/compiler.rb",
    "lib/rattler/compiler/grammar_parser.rb",
    "lib/rattler/compiler/metagrammar.rb",
    "lib/rattler/compiler/optimizer.rb",
    "lib/rattler/compiler/optimizer/composite_reducing.rb",
    "lib/rattler/compiler/optimizer/flatten_choice.rb",
    "lib/rattler/compiler/optimizer/flatten_sequence.rb",
    "lib/rattler/compiler/optimizer/flattening.rb",
    "lib/rattler/compiler/optimizer/inline_regular_rules.rb",
    "lib/rattler/compiler/optimizer/join_match_capturing_sequence.rb",
    "lib/rattler/compiler/optimizer/join_match_choice.rb",
    "lib/rattler/compiler/optimizer/join_match_matching_sequence.rb",
    "lib/rattler/compiler/optimizer/join_match_sequence.rb",
    "lib/rattler/compiler/optimizer/join_predicate_bare_match.rb",
    "lib/rattler/compiler/optimizer/join_predicate_match.rb",
    "lib/rattler/compiler/optimizer/join_predicate_nested_match.rb",
    "lib/rattler/compiler/optimizer/join_predicate_or_bare_match.rb",
    "lib/rattler/compiler/optimizer/join_predicate_or_match.rb",
    "lib/rattler/compiler/optimizer/join_predicate_or_nested_match.rb",
    "lib/rattler/compiler/optimizer/match_joining.rb",
    "lib/rattler/compiler/optimizer/optimization.rb",
    "lib/rattler/compiler/optimizer/optimization_context.rb",
    "lib/rattler/compiler/optimizer/optimization_sequence.rb",
    "lib/rattler/compiler/optimizer/optimizations.rb",
    "lib/rattler/compiler/optimizer/optimize_children.rb",
    "lib/rattler/compiler/optimizer/reduce_repeat_match.rb",
    "lib/rattler/compiler/optimizer/remove_meaningless_wrapper.rb",
    "lib/rattler/compiler/optimizer/simplify_redundant_repeat.rb",
    "lib/rattler/compiler/optimizer/simplify_token_match.rb",
    "lib/rattler/compiler/parser_generator.rb",
    "lib/rattler/compiler/parser_generator/apply_generator.rb",
    "lib/rattler/compiler/parser_generator/assert_generator.rb",
    "lib/rattler/compiler/parser_generator/attributed_sequence_generator.rb",
    "lib/rattler/compiler/parser_generator/back_reference_generator.rb",
    "lib/rattler/compiler/parser_generator/choice_generator.rb",
    "lib/rattler/compiler/parser_generator/delegating_generator.rb",
    "lib/rattler/compiler/parser_generator/disallow_generator.rb",
    "lib/rattler/compiler/parser_generator/e_symbol_generator.rb",
    "lib/rattler/compiler/parser_generator/eof_generator.rb",
    "lib/rattler/compiler/parser_generator/expr_generator.rb",
    "lib/rattler/compiler/parser_generator/fail_generator.rb",
    "lib/rattler/compiler/parser_generator/gen_method_names.rb",
    "lib/rattler/compiler/parser_generator/general_list_generator.rb",
    "lib/rattler/compiler/parser_generator/general_repeat_generator.rb",
    "lib/rattler/compiler/parser_generator/grammar_generator.rb",
    "lib/rattler/compiler/parser_generator/group_match.rb",
    "lib/rattler/compiler/parser_generator/group_match_generator.rb",
    "lib/rattler/compiler/parser_generator/label_generator.rb",
    "lib/rattler/compiler/parser_generator/list0_generating.rb",
    "lib/rattler/compiler/parser_generator/list1_generating.rb",
    "lib/rattler/compiler/parser_generator/list_generator.rb",
    "lib/rattler/compiler/parser_generator/match_generator.rb",
    "lib/rattler/compiler/parser_generator/nested.rb",
    "lib/rattler/compiler/parser_generator/node_action_generator.rb",
    "lib/rattler/compiler/parser_generator/one_or_more_generating.rb",
    "lib/rattler/compiler/parser_generator/optional_generating.rb",
    "lib/rattler/compiler/parser_generator/predicate_propogating.rb",
    "lib/rattler/compiler/parser_generator/repeat_generator.rb",
    "lib/rattler/compiler/parser_generator/rule_generator.rb",
    "lib/rattler/compiler/parser_generator/rule_set_generator.rb",
    "lib/rattler/compiler/parser_generator/semantic_action_generator.rb",
    "lib/rattler/compiler/parser_generator/sequence_generating.rb",
    "lib/rattler/compiler/parser_generator/sequence_generator.rb",
    "lib/rattler/compiler/parser_generator/skip_generator.rb",
    "lib/rattler/compiler/parser_generator/skip_propogating.rb",
    "lib/rattler/compiler/parser_generator/sub_generating.rb",
    "lib/rattler/compiler/parser_generator/super_generator.rb",
    "lib/rattler/compiler/parser_generator/token_generator.rb",
    "lib/rattler/compiler/parser_generator/token_propogating.rb",
    "lib/rattler/compiler/parser_generator/top_level.rb",
    "lib/rattler/compiler/parser_generator/zero_or_more_generating.rb",
    "lib/rattler/compiler/rattler.rtlr",
    "lib/rattler/compiler/ruby_generator.rb",
    "lib/rattler/parsers.rb",
    "lib/rattler/parsers/action_code.rb",
    "lib/rattler/parsers/analysis.rb",
    "lib/rattler/parsers/apply.rb",
    "lib/rattler/parsers/assert.rb",
    "lib/rattler/parsers/atomic.rb",
    "lib/rattler/parsers/attributed_sequence.rb",
    "lib/rattler/parsers/back_reference.rb",
    "lib/rattler/parsers/choice.rb",
    "lib/rattler/parsers/combinator_parser.rb",
    "lib/rattler/parsers/combining.rb",
    "lib/rattler/parsers/disallow.rb",
    "lib/rattler/parsers/e_symbol.rb",
    "lib/rattler/parsers/eof.rb",
    "lib/rattler/parsers/fail.rb",
    "lib/rattler/parsers/grammar.rb",
    "lib/rattler/parsers/label.rb",
    "lib/rattler/parsers/list_parser.rb",
    "lib/rattler/parsers/match.rb",
    "lib/rattler/parsers/node_action.rb",
    "lib/rattler/parsers/node_code.rb",
    "lib/rattler/parsers/parser.rb",
    "lib/rattler/parsers/parser_scope.rb",
    "lib/rattler/parsers/predicate.rb",
    "lib/rattler/parsers/repeat.rb",
    "lib/rattler/parsers/rule.rb",
    "lib/rattler/parsers/rule_set.rb",
    "lib/rattler/parsers/semantic.rb",
    "lib/rattler/parsers/semantic_action.rb",
    "lib/rattler/parsers/sequence.rb",
    "lib/rattler/parsers/sequencing.rb",
    "lib/rattler/parsers/skip.rb",
    "lib/rattler/parsers/super.rb",
    "lib/rattler/parsers/token.rb",
    "lib/rattler/rake_task.rb",
    "lib/rattler/runner.rb",
    "lib/rattler/runtime.rb",
    "lib/rattler/runtime/extended_packrat_parser.rb",
    "lib/rattler/runtime/packrat_parser.rb",
    "lib/rattler/runtime/parse_failure.rb",
    "lib/rattler/runtime/parse_node.rb",
    "lib/rattler/runtime/parser.rb",
    "lib/rattler/runtime/parser_helper.rb",
    "lib/rattler/runtime/recursive_descent_parser.rb",
    "lib/rattler/runtime/syntax_error.rb",
    "lib/rattler/util.rb",
    "lib/rattler/util/grammar_cli.rb",
    "lib/rattler/util/graphviz.rb",
    "lib/rattler/util/graphviz/digraph_builder.rb",
    "lib/rattler/util/graphviz/node_builder.rb",
    "lib/rattler/util/line_counter.rb",
    "lib/rattler/util/node.rb",
    "lib/rattler/util/parser_cli.rb",
    "lib/rattler/util/parser_spec_helper.rb"
  ]
  s.homepage = "http://github.com/jarhart/rattler"
  s.licenses = ["MIT"]
  s.require_paths = ["lib"]
  s.rubygems_version = "1.8.24"
  s.summary = "Ruby Tool for Language Recognition"
  s.test_files = ["features/command_line", "features/command_line/dest_option.feature", "features/command_line/lib_option.feature", "features/command_line/output_option.feature", "features/command_line/rtlr.feature", "features/error_reporting", "features/error_reporting/automatic_error_messages.feature", "features/error_reporting/custom_error_messages.feature", "features/examples", "features/examples/json_parser.markdown", "features/extended_matching_syntax", "features/extended_matching_syntax/back_reference.feature", "features/extended_matching_syntax/e_symbol.feature", "features/extended_matching_syntax/eof.feature", "features/extended_matching_syntax/fail.feature", "features/extended_matching_syntax/fragments.feature", "features/extended_matching_syntax/include.feature", "features/extended_matching_syntax/list_matching.feature", "features/extended_matching_syntax/posix_class.feature", "features/extended_matching_syntax/repeat.feature", "features/extended_matching_syntax/skip_operator.feature", "features/extended_matching_syntax/super.feature", "features/extended_matching_syntax/token.feature", "features/extended_matching_syntax/whitespace.feature", "features/extended_matching_syntax/word_literal.feature", "features/grammar_heading", "features/grammar_heading/explicit_start_rule.feature", "features/grammar_heading/grammar_declaration.feature", "features/grammar_heading/include.feature", "features/grammar_heading/require.feature", "features/peg_syntax", "features/peg_syntax/any_character.feature", "features/peg_syntax/character_class.feature", "features/peg_syntax/comments.feature", "features/peg_syntax/literal.feature", "features/peg_syntax/negative_lookahead.feature", "features/peg_syntax/nonterminal.feature", "features/peg_syntax/one_or_more.feature", "features/peg_syntax/optional.feature", "features/peg_syntax/ordered_choice.feature", "features/peg_syntax/positive_lookahead.feature", "features/peg_syntax/sequence.feature", "features/peg_syntax/start_rule.feature", "features/peg_syntax/zero_or_more.feature", "features/README.markdown", "features/semantics", "features/semantics/labels.feature", "features/semantics/negative_semantic_predicate.feature", "features/semantics/node_action.feature", "features/semantics/positive_semantic_predicate.feature", "features/semantics/semantic_action.feature", "features/semantics/semantic_result.feature", "features/semantics/side_effect.feature", "features/step_definitions", "features/step_definitions/cli_steps.rb", "features/step_definitions/grammar_steps.rb", "features/support", "features/support/env.rb", "features/Tutorial.md", "spec/rattler", "spec/rattler/compiler", "spec/rattler/compiler/assert_compiler_examples.rb", "spec/rattler/compiler/attributed_sequence_compiler_examples.rb", "spec/rattler/compiler/disallow_compiler_examples.rb", "spec/rattler/compiler/grammar_parser_spec.rb", "spec/rattler/compiler/optimizer", "spec/rattler/compiler/optimizer/flatten_choice_spec.rb", "spec/rattler/compiler/optimizer/flatten_sequence_spec.rb", "spec/rattler/compiler/optimizer/inline_regular_rules_spec.rb", "spec/rattler/compiler/optimizer/join_match_capturing_sequence_spec.rb", "spec/rattler/compiler/optimizer/join_match_choice_spec.rb", "spec/rattler/compiler/optimizer/join_match_matching_sequence_spec.rb", "spec/rattler/compiler/optimizer/join_predicate_bare_match_spec.rb", "spec/rattler/compiler/optimizer/join_predicate_nested_match_spec.rb", "spec/rattler/compiler/optimizer/join_predicate_or_bare_match_spec.rb", "spec/rattler/compiler/optimizer/join_predicate_or_nested_match_spec.rb", "spec/rattler/compiler/optimizer/reduce_repeat_match_spec.rb", "spec/rattler/compiler/optimizer/remove_meaningless_wrapper_spec.rb", "spec/rattler/compiler/optimizer/simplify_redundant_repeat_spec.rb", "spec/rattler/compiler/optimizer/simplify_token_match_spec.rb", "spec/rattler/compiler/optimizer_spec.rb", "spec/rattler/compiler/parser_generator", "spec/rattler/compiler/parser_generator/apply_generator_spec.rb", "spec/rattler/compiler/parser_generator/assert_generator_spec.rb", "spec/rattler/compiler/parser_generator/attributed_sequence_generator_spec.rb", "spec/rattler/compiler/parser_generator/back_reference_generator_spec.rb", "spec/rattler/compiler/parser_generator/choice_generator_spec.rb", "spec/rattler/compiler/parser_generator/disallow_generator_spec.rb", "spec/rattler/compiler/parser_generator/e_symbol_generator_spec.rb", "spec/rattler/compiler/parser_generator/eof_generator_spec.rb", "spec/rattler/compiler/parser_generator/fail_generator_spec.rb", "spec/rattler/compiler/parser_generator/grammar_generator_spec.rb", "spec/rattler/compiler/parser_generator/group_match_generator_spec.rb", "spec/rattler/compiler/parser_generator/group_match_spec.rb", "spec/rattler/compiler/parser_generator/label_generator_spec.rb", "spec/rattler/compiler/parser_generator/list0_generator_examples.rb", "spec/rattler/compiler/parser_generator/list1_generator_examples.rb", "spec/rattler/compiler/parser_generator/list_generator_spec.rb", "spec/rattler/compiler/parser_generator/match_generator_spec.rb", "spec/rattler/compiler/parser_generator/node_action_generator_spec.rb", "spec/rattler/compiler/parser_generator/one_or_more_generator_examples.rb", "spec/rattler/compiler/parser_generator/optional_generator_examples.rb", "spec/rattler/compiler/parser_generator/repeat_generator_spec.rb", "spec/rattler/compiler/parser_generator/rule_generator_spec.rb", "spec/rattler/compiler/parser_generator/rule_set_generator_spec.rb", "spec/rattler/compiler/parser_generator/semantic_action_generator_spec.rb", "spec/rattler/compiler/parser_generator/sequence_generator_spec.rb", "spec/rattler/compiler/parser_generator/skip_generator_spec.rb", "spec/rattler/compiler/parser_generator/super_generator_spec.rb", "spec/rattler/compiler/parser_generator/token_generator_spec.rb", "spec/rattler/compiler/parser_generator/zero_or_more_generator_examples.rb", "spec/rattler/compiler/ruby_generator_spec.rb", "spec/rattler/compiler/semantic_action_compiler_examples.rb", "spec/rattler/compiler/shared_compiler_examples.rb", "spec/rattler/compiler/skip_compiler_examples.rb", "spec/rattler/compiler/token_compiler_examples.rb", "spec/rattler/compiler_spec.rb", "spec/rattler/parsers", "spec/rattler/parsers/action_code_spec.rb", "spec/rattler/parsers/analysis_spec.rb", "spec/rattler/parsers/apply_spec.rb", "spec/rattler/parsers/assert_spec.rb", "spec/rattler/parsers/attributed_sequence_spec.rb", "spec/rattler/parsers/back_reference_spec.rb", "spec/rattler/parsers/choice_spec.rb", "spec/rattler/parsers/combinator_parser_spec.rb", "spec/rattler/parsers/disallow_spec.rb", "spec/rattler/parsers/e_symbol_spec.rb", "spec/rattler/parsers/eof_spec.rb", "spec/rattler/parsers/fail_spec.rb", "spec/rattler/parsers/grammar_spec.rb", "spec/rattler/parsers/label_spec.rb", "spec/rattler/parsers/list_parser_spec.rb", "spec/rattler/parsers/match_spec.rb", "spec/rattler/parsers/node_action_spec.rb", "spec/rattler/parsers/parser_scope_spec.rb", "spec/rattler/parsers/repeat_spec.rb", "spec/rattler/parsers/rule_set_spec.rb", "spec/rattler/parsers/semantic_action_spec.rb", "spec/rattler/parsers/sequence_spec.rb", "spec/rattler/parsers/skip_spec.rb", "spec/rattler/parsers/super_spec.rb", "spec/rattler/parsers/token_spec.rb", "spec/rattler/runtime", "spec/rattler/runtime/extended_packrat_parser_spec.rb", "spec/rattler/runtime/packrat_parser_spec.rb", "spec/rattler/runtime/parse_failure_spec.rb", "spec/rattler/runtime/parse_node_spec.rb", "spec/rattler/runtime/parser_spec.rb", "spec/rattler/runtime/recursive_descent_parser_spec.rb", "spec/rattler/runtime/shared_parser_examples.rb", "spec/rattler/util", "spec/rattler/util/graphviz", "spec/rattler/util/graphviz/node_builder_spec.rb", "spec/rattler/util/line_counter_spec.rb", "spec/rattler/util/node_spec.rb", "spec/rattler_spec.rb", "spec/spec_helper.rb", "spec/support", "spec/support/combinator_parser_spec_helper.rb", "spec/support/compiler_spec_helper.rb", "spec/support/parser_generator_spec_helper.rb", "spec/support/runtime_parser_spec_helper.rb"]

  if s.respond_to? :specification_version then
    s.specification_version = 3

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_development_dependency(%q<bundler>, ["~> 1.1"])
      s.add_development_dependency(%q<ruby-graphviz>, ["~> 1.0"])
      s.add_development_dependency(%q<jeweler>, ["~> 1.8"])
      s.add_development_dependency(%q<rspec>, ["~> 2.10"])
      s.add_development_dependency(%q<cucumber>, ["~> 1.2"])
      s.add_development_dependency(%q<aruba>, [">= 0"])
      s.add_development_dependency(%q<rcov>, [">= 0"])
      s.add_development_dependency(%q<yard>, [">= 0"])
      s.add_development_dependency(%q<guard>, [">= 0"])
      s.add_development_dependency(%q<guard-rspec>, [">= 0"])
      s.add_development_dependency(%q<guard-cucumber>, [">= 0"])
      s.add_development_dependency(%q<rb-notifu>, [">= 0"])
      s.add_development_dependency(%q<win32console>, [">= 0"])
    else
      s.add_dependency(%q<bundler>, ["~> 1.1"])
      s.add_dependency(%q<ruby-graphviz>, ["~> 1.0"])
      s.add_dependency(%q<jeweler>, ["~> 1.8"])
      s.add_dependency(%q<rspec>, ["~> 2.10"])
      s.add_dependency(%q<cucumber>, ["~> 1.2"])
      s.add_dependency(%q<aruba>, [">= 0"])
      s.add_dependency(%q<rcov>, [">= 0"])
      s.add_dependency(%q<yard>, [">= 0"])
      s.add_dependency(%q<guard>, [">= 0"])
      s.add_dependency(%q<guard-rspec>, [">= 0"])
      s.add_dependency(%q<guard-cucumber>, [">= 0"])
      s.add_dependency(%q<rb-notifu>, [">= 0"])
      s.add_dependency(%q<win32console>, [">= 0"])
    end
  else
    s.add_dependency(%q<bundler>, ["~> 1.1"])
    s.add_dependency(%q<ruby-graphviz>, ["~> 1.0"])
    s.add_dependency(%q<jeweler>, ["~> 1.8"])
    s.add_dependency(%q<rspec>, ["~> 2.10"])
    s.add_dependency(%q<cucumber>, ["~> 1.2"])
    s.add_dependency(%q<aruba>, [">= 0"])
    s.add_dependency(%q<rcov>, [">= 0"])
    s.add_dependency(%q<yard>, [">= 0"])
    s.add_dependency(%q<guard>, [">= 0"])
    s.add_dependency(%q<guard-rspec>, [">= 0"])
    s.add_dependency(%q<guard-cucumber>, [">= 0"])
    s.add_dependency(%q<rb-notifu>, [">= 0"])
    s.add_dependency(%q<win32console>, [">= 0"])
  end
end

